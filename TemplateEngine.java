...
public class TemplateEngine {
    ...
    private Result instantiate(String instancedString, ArrayList<Template> sortedTemplates, ArrayList<EntryMap.Entry> sortedEntries, Integer matchingMode){
        ...
        Integer baseYear = Calendar.getInstance().get(Calendar.YEAR);
        
        for (Integer i=0; i<sortedTemplates.size(); i++){
            currentTemplate = sortedTemplates.get(i);
            replaceHappened = Boolean.FALSE;
            
            //Does the template match year? 
            if(templateMatch(currentTemplate, "year", matchingMode)) {
                //System.out.println(currentTemplate.getContent());
              
                //If we find the base_year entry as well, check if it is valid
                for(Integer j=0; j<sortedEntries.size(); j++){
                    currentEntry = sortedEntries.get(j);
                    if (entryMatch(currentEntry, "base_year", matchingMode)) {
                ...
                //Go through all the entries to find the year entry
                for(Integer j=0; j<sortedEntries.size(); j++) {
                    currentEntry = sortedEntries.get(j);
                    
                    //Checking if we have found the year entry 
                    if(isAMatch(currentTemplate, currentEntry, matchingMode)) {
                        
                        //Template 1
                        if(currentEntry.getValue().matches("\\d+ years ago")) {
                            String x = currentEntry.getValue().substring(0, currentEntry.getValue().indexOf(" "));
                        ...
                        //Template 2
                        if(currentEntry.getValue().matches("in \\d+ years")) {
                            String x = currentEntry.getValue().substring(3, currentEntry.getValue().indexOf(" ", 3));
    ...
    private Boolean templateMatch(Template template, String str, Integer matchingMode) {
        String leftHandSide;
        String rightHandSide;
        if (!accurateSearch(matchingMode)) {
            leftHandSide = template.getContent().replaceAll("\\s","");
            rightHandSide = str.replaceAll("\\s","");
        } else {
    ...
    private Boolean entryMatch(EntryMap.Entry entry, String str, Integer matchingMode){
        String leftHandSide;
        String rightHandSide;
        if (!accurateSearch(matchingMode)) {
            leftHandSide = str.replaceAll("\\s","");
